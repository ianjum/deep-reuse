...
struct TransformFill {
  void operator()(const Device& d,
    ...
    std::cout << "in Fill conv_2d.h" << std::endl;  
...
// Add by Lin
template <typename Device, typename T> 
struct CalPreCluster {
  void operator() (const Device& d, 
                   typename TTypes<T, 2>::ConstTensor input,
                   const int32 dimension, int N,
		   typename TTypes<T, 1>::Tensor output) {
    const int rows = input.dimension(0);
//     Eigen::array<T, 1> in_max;
//     Eigen::array<T, 1> in_min;
//     in_max[0] = rows;
//     in_min[0] = rows;
//     in_max = input.argmax(dimension);
//     in_min = input.argmin(dimension);
    output.device(d) = input.argmax(dimension).template cast<T>();
//     output -= input.argmin(dimension);
//     output = output/N;
//     output.device(d) = output.template cast<T>();
  }
};
...
struct PadInput {
  void operator()(const Device& d,
                  ...
                  typename TTypes<T, NDIMS, IndexType>::Tensor out,
                  TensorFormat format) {
    std::cout << "PadInput 2d.h" << std::endl;
